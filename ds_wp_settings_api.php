...
if (!class_exists('DustySun\WP_Settings_API\v2\Options')) {
    final class Options {
        ...
        public static function section(string $text_domain, string $section_key): array {
            $main   = self::main($text_domain);
            $suffix = $main['options_suffix'] ?? '';
            $optKey = $section_key . $suffix;
            if (!isset(self::$cache[$optKey])) {
                self::$cache[$optKey] = get_option($optKey, []);
            }
            return self::$cache[$optKey];
        }

        /**
         * Get a single field value with a default.
         */
        public static function get(string $text_domain, string $section_key, string $field_id, $default = '') {
            $section = self::section($text_domain, $section_key);
            return array_key_exists($field_id, $section) ? $section[$field_id] : $default;
        }
    }
}
if (!class_exists('DustySun\WP_Settings_API\v2\SettingsBuilder')) {
    class SettingsBuilder {
        ...
        public function set_main_settings($update_db = false, $reset_defaults = false) {
            ...
            // If JSON didn’t provide a version, fill it from the host plugin
            if (empty($this->main_settings['version']) || $this->main_settings['version'] == '') {
                $this->main_settings['version'] = $this->resolve_host_version();
            }
        ...
        private function resolve_host_version(): string {
            ...
            // Try plugin header (admin loads already include this; otherwise include it defensively)
            if ($this->ds_wp_api_settings_init_data['plugin_file']) {
                if (!function_exists('get_plugin_data')) {
                    @include_once ABSPATH . 'wp-admin/includes/plugin.php';
                }
                if (function_exists('get_plugin_data')) {
                    $data = get_plugin_data($this->ds_wp_api_settings_init_data['plugin_file'], false, false);
                    if (!empty($data['Version'])) {
                        return $data['Version'];
                    }
                }

                // Fallback: get_file_data (lighter)
                if (!function_exists('get_file_data')) {
                    @include_once ABSPATH . 'wp-includes/functions.php';
                }
                if (function_exists('get_file_data')) {
                    $headers = get_file_data($this->ds_wp_api_settings_init_data['plugin_file'], ['Version' => 'Version']);
	...
	<?php
		...
		<?php
				...
				<h2 class="nav-tab-wrapper" id="panel-nav-tab">
                    ...
                    foreach ($ds_wp_settings_api_tab_array as $ds_wp_settings_api_section_key => $ds_wp_settings_api_section) {
                        ...
                        ?>
							<a href="<?php menu_page_url($this->main_settings['page_slug']);?>&tab=<?php echo $ds_wp_settings_api_section_slug; ?>#panel-nav-tab" class="nav-tab <?php if ($active_tab == $ds_wp_settings_api_section_slug) {
							    echo 'nav-tab-active';
							} ?>"><?php _e($tab_label, $this->main_settings['text_domain']); ?></a>
        ...
        public static function decrypt_data($encoded_input, $password = null) {
            ...
            if (hash_hmac('sha256', $cipher_input, $openssl_key, true) != $openssl_hash) {
                return null;
            }
        ...
        public function ds_wp_settings_api_sanitize( $raw_input_data_fields ) {
            ...
            if ($sanitization_fields == '') {
                ...
                foreach ($sanitization_fields as $sanitization_field) {
                    ...
                    } elseif ($validation_type === 'post_select') {
                        $raw = $raw_input_data_fields[$sanitization_field['id']] ?? null;
                        if (is_array($raw)) {
                            $validated_info = array_values(array_unique(array_map('absint', $raw)));
                        } else {
        ...
        /**
         * Returns the value of a field from the current settings.
         *
         * @param string $section_key The key of the section.
         * @param string $field_id The ID of the field.
         * @param mixed $default The default value to return if the field is not set. Default is an empty string.
         * @return mixed The value of the field, or the default value if the field is not set.
         */
        public function get_field(string $section_key, string $field_id, $default = '') {
            $optKey = $section_key . ($this->main_settings['options_suffix'] ?? '');
            if (!isset($this->current_settings[$optKey])) {
                return $default;
            }
            $val = $this->current_settings[$optKey][$field_id] ?? $default;
            return $val;
        }
    }
